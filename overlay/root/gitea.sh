...
setenv LFS_JWT_SECRET `cat /root/LFS_JWT_SECRET`
...
sed -i .bak -e "s/PASSWD   =/PASSWD   = ${PASS}/" ${CFG}
sed -i .bak -e "s/USER     = root/USER     = ${DBUSER}/" ${CFG}
sed -i .bak -e "s/NAME     = gitea/NAME     = ${DBNAME}/" ${CFG}
